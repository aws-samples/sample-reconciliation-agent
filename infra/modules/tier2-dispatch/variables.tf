...
variable "otel_layer_arn" {
  description = <<-EOT
    ADOT Python layer ARN for the dispatcher. Empty disables client-side tracing entirely.

    The dispatcher is the OTel participant that matters most on the runtime backend: since Tier-2 async
    dispatch it is what actually calls InvokeAgentRuntime, so without the layer here the trace that
    links a map run to the agent's own spans simply has no client end. assets/agent-evaluation.md
    describes the agent-worker closing that gap, which is now only true of the harness and retry paths.
  EOT
  type        = string
  default     = ""
}
...
variable "otel_baggage_span_attribute_keys" {
  description = "W3C baggage keys promoted onto spans. Must match the tier1 and harness modules' copies."
  type        = string
