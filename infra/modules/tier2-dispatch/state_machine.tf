...
locals {
  ...
  tier2_definition = jsonencode({
    ...
    StartAt = "StartRun"
    States = {
      # Entry point sets the drain-pass counter, then falls through to Collect. A `Pass` rather than
      # folding it into Collect's output: the counter has to survive the loop below, and Collect
      # overwrites $.run on every pass.
      StartRun = {
        Type       = "Pass"
        Result     = { pass = 0 }
        ResultPath = "$.drain"
      ...
      # ⚠️ The DRAIN loop, and the reason a submission does not wait for the next tick. Without it a
      # run investigates one collected batch and exits, so anything that arrived while it was working
      # -- or anything over the per-run ceiling -- sat PENDING until the schedule fired again. With the
      # single-flight guard that made the schedule interval the real latency floor for the whole queue,
      # not just for an idle one.
      CountPass = {
        Type = "Pass"
        Parameters = {
          "pass.$" = "States.MathAdd($.drain.pass, 1)"
        }
        ResultPath = "$.drain"
        Next       = "MoreToDo"
      }

      # Bounded, because an unbounded self-loop is how a stuck case becomes an execution that runs for
      # a year. A case that cannot leave PENDING would otherwise be collected forever; the cap turns
      # that into a run that ends and a backlog the schedule retries.
      MoreToDo = {
        Type = "Choice"
        Choices = [{
          Variable           = "$.drain.pass"
          NumericGreaterThan = 12
          Next               = "DrainCapReached"
        }]
        Default = "Collect"
      }

      DrainCapReached = { Type = "Succeed" }
