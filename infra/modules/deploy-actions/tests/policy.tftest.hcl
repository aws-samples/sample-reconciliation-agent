...
# bucket -- every recon-only deployment -- it renders BYTE FOR BYTE the statements it rendered
...
run "with_no_additional_bucket_the_policy_is_byte_for_byte_what_it_always_was" {
  ...
  assert {
    ...
    error_message = "with additional_assets_bucket_arns = [] the actor policy must render exactly the five statements it rendered before that input existed; a deployed recon environment would otherwise plan an update of aws_iam_role_policy.actions"
  }

  assert {
    condition     = length(jsondecode(aws_iam_role_policy.actions.policy).Statement) == 5
    error_message = "five statements, no more: nothing may be appended when there is no additional bucket"
  }
}
