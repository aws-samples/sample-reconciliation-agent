...
#
# Both are instances of modules/agentcore-memory, the module recon's lessons memory also uses. The
# knowledge memory creates the execution role AgentCore Memory assumes for the edge_cases
# extraction/consolidation passes; the chat memory has no strategy and never invokes a model, but the
# argument is required on the resource, so it attaches that same role rather than a second one. The
# `moved` blocks at the end carry a deployment's existing resources to the module addresses with
...
# email. The module explains why only EXTRACTION is overridden and that `append_to_prompt` REPLACES
# the default instructions despite its name, which is why the prompt below is a complete instruction
# set.
module "knowledge_memory" {
  ...
  strategy = {
    ...
    # Byte for byte the prompt the strategy was created with, at the same indentation (the `<<-`
    # form strips the common leading whitespace); a change here is an in-place update of the live
    # strategy.
...
module "chat_memory" {
  ...
  event_expiry_days = 7 # days
  region            = local.region
  account_id        = local.account_id

  # The knowledge memory's role, not a second one (see the header).
  create_execution_role = false
  execution_role_arn    = module.knowledge_memory.execution_role_arn
}

# State-only moves of the resources declared inline in this file until 2026-09: same names, same
# arguments.
moved {
  from = aws_iam_role.memory
  to   = module.knowledge_memory.aws_iam_role.memory[0]
}

moved {
  from = aws_iam_role_policy.memory
  to   = module.knowledge_memory.aws_iam_role_policy.memory[0]
}

moved {
  from = aws_bedrockagentcore_memory.knowledge
  to   = module.knowledge_memory.aws_bedrockagentcore_memory.this
}

moved {
  from = aws_bedrockagentcore_memory.chat
  to   = module.chat_memory.aws_bedrockagentcore_memory.this
}

moved {
  from = aws_bedrockagentcore_memory_strategy.edge_cases
  to   = module.knowledge_memory.aws_bedrockagentcore_memory_strategy.this[0]
}
