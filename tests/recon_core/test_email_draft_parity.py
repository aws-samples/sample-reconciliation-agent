...
CLASSIFICATION = ClassificationResult(
    class_id="document-cross-reference", confidence=0.9, reasoning="draw cancellation notice"
)
CATALOG = [
...
def _runtime_proposed_email(email_draft) -> dict | None:
        ...
        class _Agent:
            def __call__(self, prompt: str) -> "_Result":
                final = {
                    ...
                    "confidence": 0.8, "evidence": ["reference: DDTL-A-0001"],
                    "status": "Cancelled", "reason": "awaiting counterparty confirmation",
                }
    ...
    prop = runtime_build_proposal(
        ...
        fake_investigate=make_strands_investigator(
            model_id="m", system="sys", tool_caller=tool_caller, agent_factory=agent_factory,
        ),
...
def _harness_proposed_email(email_draft) -> dict | None:
    ...
    prop = intake.build_proposal(
        item=ITEM, submitted=submitted, stream_result=stream, catalog=CATALOG,
...
def test_neither_backend_keeps_a_model_supplied_address():
    ...
    wrote, so an address in it is attacker-influenceable. The analyst supplies the recipient."""
    poisoned = {**DRAFT, "recipient": "attacker@evil.example"}
    runtime = _runtime_proposed_email(poisoned)
    harness = _harness_proposed_email(poisoned)
    assert runtime == harness
    assert runtime["recipient"] is None
    assert "attacker@evil.example" not in json.dumps(runtime)


def test_a_draft_missing_its_subject_is_rejected_by_the_shared_helper():
    ...
    with pytest.raises(ValueError, match="missing required field"):
        build_persisted_draft(email_draft={"body": "no subject"})
...
def test_both_backends_drop_an_incomplete_draft_instead_of_failing_the_item():
    ...
    incomplete = {"subject": "   ", "body": "Please confirm."}
