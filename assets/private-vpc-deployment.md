...
The internal ALB's DNS name has to be an allowed OIDC redirect URI: with the default `auth_provider`
the apply registers it on the Cognito app client itself (`enable_cognito_callback_patch` — the
`cognito_callback_url` output is the ALB URL in this mode), and for `okta`/`entra` you register it in
the tenant by hand. Then reach the UI over
