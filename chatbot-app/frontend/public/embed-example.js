  ...
  iframes.forEach((iframe, index) => {
    iframe.addEventListener("load", function () {
      ...
      updateStatus(
        iframe.id,
        "checking",
        "Iframe loaded, checking authentication...",
      );
    });
...
function updateStatus(iframeId, status, message) {
  ...
  // Use textContent (not innerHTML) so the message is never parsed as HTML — prevents XSS,
  // matching updateAllStatuses() below.
...
function reloadIframe(iframeId) {
  ...
  if (iframe) {
    updateStatus(
      iframeId,
      "checking",
      '<span class="loading"></span> Reloading iframe...',
    );
  ...
  if (iframe && iframe.contentWindow) {
    try {
      ...
      updateStatus(
        iframeId,
        "checking",
        '<span class="loading"></span> Testing authentication...',
      );
