/**
 * Browser-side Authorization header for the pipeline BFF.
 *
...
import { authHeaders, pipelineFetch, pipelineIdToken } from "@/lib/pipeline-auth";
...
describe("pipeline-auth", () => {
  ...
  it("never throws when token acquisition fails", async () => {
    ...
    expect(await pipelineIdToken()).toBeNull();
    expect(await authHeaders()).toEqual({});
  });

  it("pipelineFetch attaches the header and preserves caller headers and init", async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal("fetch", fetchMock);

    await pipelineFetch("/api/pipeline/config", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: "{}",
    });

    expect(fetchMock).toHaveBeenCalledWith("/api/pipeline/config", {
      method: "PUT",
      body: "{}",
      headers: {
  ...
  it("pipelineFetch still sends the request when unauthenticated (the server decides)", async () => {
    acquireTokenSilent.mockResolvedValue({ idToken: undefined });
    const fetchMock = vi.fn().mockResolvedValue({ ok: false, status: 401 });
    vi.stubGlobal("fetch", fetchMock);

    await pipelineFetch("/api/pipeline/deals");

    expect(fetchMock).toHaveBeenCalledWith("/api/pipeline/deals", { headers: {} });
  });
});
