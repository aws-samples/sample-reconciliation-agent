...
// Mock the access-token helper that AssistantTurn uses to attach Authorization headers.
vi.mock("@/lib/access-token", () => ({
  getAccessToken: vi.fn().mockResolvedValue("mock-token"),
  getAuthHeaders: vi
    .fn()
    .mockResolvedValue({ Authorization: "Bearer mock-token" }),
  getCurrentUserId: vi.fn().mockResolvedValue("mock-user-oid"),
}));
...
vi.mock("@/components/ui/LazyImage", () => ({
  LazyImage: ({ src }: { src: string }) => (
    <img data-testid="lazy-image" src={src} />
  ),
}));
...
describe("AssistantTurn", () => {
  ...
  describe("Latency Metrics Display", () => {
    it("should display TTFT when available", () => {
      ...
      const { container } = render(
        <AssistantTurn messages={messages} sessionId="test-session" />,
      );
  ...
  describe("Empty State", () => {
    it("should return null when no messages", () => {
      const { container } = render(
        <AssistantTurn messages={[]} sessionId="test-session" />,
      );
  ...
  describe("Component Rendering Order", () => {
    ...
    it("should preserve order when tool interrupts text stream", () => {
      ...
      expect(
        container.querySelector('[data-testid="tool-execution"]'),
      ).toBeDefined();
    });
