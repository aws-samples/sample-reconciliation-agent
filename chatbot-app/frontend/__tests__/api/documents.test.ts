...
describe("Documents Download API", () => {
  ...
  describe("User ID Extraction from Authorization Header", () => {
    ...
    it("should extract userId from authenticated request", () => {
      ...
      mockExtract.mockReturnValue({
        userId: "18c1e380-1234-5678-9abc-def012345678",
      });

      const mockRequest = {
        headers: {
          get: (name: string) =>
            name === "authorization" ? "Bearer mock-jwt-token" : null,
        },
      };
    ...
    it("should use full UUID from JWT (not truncated sessionId prefix)", () => {
      ...
      const mockRequest = {
        headers: {
          get: (name: string) =>
            name === "authorization" ? "Bearer jwt-with-full-uuid" : null,
        },
      };
  ...
  describe("S3 Key Construction", () => {
    it("should construct correct S3 key for word document", () => {
      ...
      expect(s3Key).toBe(
        "s3://test-document-bucket/documents/18c1e380-1234-5678-9abc-def012345678/18c1e380_timestamp_uuid/word/report.docx",
      );
    });
    ...
    it("should construct correct S3 key for excel document", () => {
      ...
      expect(s3Key).toBe(
        "s3://test-document-bucket/documents/user-456-full-uuid-here/user456_xyz_session789/excel/data.xlsx",
      );
    });
    ...
    it("should construct correct S3 key for anonymous user", () => {
      ...
      expect(s3Key).toBe(
        "s3://test-document-bucket/documents/anonymous/anon0000_abc_session123/word/anonymous-doc.docx",
      );
    });
    ...
    it("should handle filenames with spaces", () => {
      ...
      expect(s3Key).toBe(
        "s3://test-document-bucket/documents/user-123-full-uuid/session-456/word/My Report 2024.docx",
      );
    });
  ...
  describe("End-to-End Flow (with extractUserFromRequest)", () => {
    it("should correctly process authenticated user download request", () => {
      ...
      const documentType =
        toolTypeToDocType[request.toolType] || request.toolType;
      ...
      expect(s3Key).toBe(
        "s3://test-document-bucket/documents/18c1e380-1234-5678-9abc-def012345678/18c1e380_timestamp_uuid/word/quarterly-report.docx",
      );
    });
    ...
    it("should correctly process anonymous user download request", () => {
      ...
      expect(s3Key).toBe(
        "s3://test-document-bucket/documents/anonymous/anon0000_timestamp_uuid/excel/untitled.xlsx",
      );
    });
  });
  ...
  describe("Frontend Integration (Authorization Header)", () => {
    ...
    it("should demonstrate correct frontend request pattern", async () => {
      ...
      // Opaque placeholder, deliberately NOT JWT-shaped so secret scanners don't flag a fake
      // fixture (the test only checks the header is passed through verbatim, never decodes it).
      const mockToken = "test-bearer-token-placeholder";
