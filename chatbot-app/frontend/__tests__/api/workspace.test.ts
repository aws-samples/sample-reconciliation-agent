...
describe("Workspace Files API", () => {
  ...
  describe("Response Formatting", () => {
    it("should format file response correctly", () => {
      ...
      expect(formattedFile).toEqual({
        ...
        s3_key:
          "s3://test-bucket/documents/user-123/session-456/word/report.docx",
    ...
    it("should map doc type to correct tool_type format using DOC_TYPE_TO_TOOL_TYPE", () => {
      ...
      expect(DOC_TYPE_TO_TOOL_TYPE["powerpoint"]).toBe(
  ...
  describe("Error Handling", () => {
    ...
    it("should handle empty S3 response", () => {
      ...
      // Route returns an empty files array when S3 has no Contents.
      const files: string[] = s3Response.Contents ?? [];
...
describe("Frontend Workspace File Fetching (useStreamEvents)", () => {
      ...
      expect(documents).toEqual([
        {
          filename: "report.docx",
          tool_type: "word",
          user_id: "18c1e380-1234-5678-9abc-def012345678",
        },
        {
          filename: "data.xlsx",
          tool_type: "excel",
  ...
  describe("Workspace API fetch headers (Integration)", () => {
    ...
    it("should include Authorization header when auth token is available", async () => {
      ...
      // fixture (the test only passes it through as a Bearer header, never decodes it).
      const authToken = "test-bearer-token-placeholder";
    ...
    it("should find files when correct headers are included", async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            files: [{ filename: "report.docx", tool_type: "word_document" }],
