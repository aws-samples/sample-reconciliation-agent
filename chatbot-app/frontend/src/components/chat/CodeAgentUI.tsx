...
export const isCodeAgentExecution = (toolExec: ToolExecution): boolean =>
  ...
  (toolExec.toolName === "skill_executor" &&
    toolExec.toolInput?.tool_name === "code_agent");
...
export const CodeAgentTerminal = React.memo<CodeAgentTerminalProps>(
    ...
    return (
      <div className="mt-1 rounded-md border border-gray-300 dark:border-white/10 bg-gray-900 dark:bg-[#0d1117] overflow-hidden shadow-sm">
        ...
        {isExpanded && (
          <div
            ref={scrollRef}
            className="px-3 py-2 max-h-40 overflow-y-auto scrollbar-thin"
          >
            {steps.map((step, i) => {
              const isHeartbeat =
              ...
              return (
                <div
                  key={i}
                  className="flex gap-2 text-[12px] leading-relaxed font-mono"
                >
                  <span
                    className={`select-none flex-shrink-0 ${isHeartbeat ? "text-blue-400" : "text-green-500/70"}`}
                  >
                    {isHeartbeat ? "●" : "$"}
                  </span>
                  <span
                    className={`whitespace-pre-wrap break-words ${isHeartbeat ? "text-gray-400 italic" : "text-gray-300"}`}
                  >
...
export const CodeAgentDetails = React.memo<CodeAgentDetailsProps>(
  ({ toolExecution }) => (
    <>
      {/* Todo checklist */}
          ...
          {toolExecution.codeTodos.map((todo) => (
            ...
            >
              {todo.status === "completed" ? (
                <svg
                  className="h-3 w-3 mt-0.5 text-green-500 flex-shrink-0"
                  viewBox="0 0 12 12"
                  fill="currentColor"
                >
                  <path
                    fillRule="evenodd"
                    d="M10.22 2.22a.75.75 0 0 1 0 1.06L4.97 8.53 1.78 5.34a.75.75 0 0 1 1.06-1.06l2.13 2.13 4.19-4.19a.75.75 0 0 1 1.06 0Z"
                    clipRule="evenodd"
                  />
                </svg>
                ...
                <svg
                  className="h-3 w-3 mt-0.5 text-muted-foreground flex-shrink-0"
                  viewBox="0 0 12 12"
                  fill="none"
                >
                  <rect
        ...
        toolExecution.codeResultMeta.files_changed.length > 0 && (
          <div className="space-y-1">
            <div className="text-caption text-muted-foreground font-medium">
              {toolExecution.codeResultMeta.files_changed.length} file
              {toolExecution.codeResultMeta.files_changed.length !== 1
                ? "s"
                : ""}{" "}
              changed
              {toolExecution.codeResultMeta.steps > 0 && (
                <span className="font-normal">
                  {" "}
                  · {toolExecution.codeResultMeta.steps} steps
                </span>
              )}
            </div>
            <div className="space-y-0.5">
              {toolExecution.codeResultMeta.files_changed.map((f) => (
                <div
                  key={f}
                  className="flex items-center gap-1.5 text-caption font-mono text-muted-foreground"
                >
                  <svg
...
export const CodeAgentResult = React.memo<CodeAgentResultProps>(
  ({ toolResult }) => (
...
export const CodeAgentDownloadButton = ({
  sessionId,
}: CodeAgentDownloadButtonProps) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!files || files.length === 0) {
        alert(
          "No workspace files found. The code agent may not have created any files yet.",
        );
        return;
      }
