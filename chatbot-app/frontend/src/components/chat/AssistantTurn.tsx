...
const parseArtifactMessage = (
  text: string,
): { title: string; wordCount: number } | null => {
  // Try with markdown bold first
  let match = text.match(
    /Document \*\*(.+?)\*\* has been created\.\s*\((\d+) words\)/,
  );
  if (match) {
...
const ArtifactNotification = ({
  ...
  const handleClick = () => {
    window.dispatchEvent(
      new CustomEvent("open-artifact-by-title", { detail: { title } }),
    );
  };
...
export const AssistantTurn = React.memo<AssistantTurnProps>(
  ({
    messages,
    currentReasoning,
    sessionId,
    ...
    const latencyMetrics = sortedMessages.find(
      (msg) => msg.latencyMetrics,
    )?.latencyMetrics;
    ...
    return (
      <div className="flex justify-start mb-8 group">
        <div
          className={`flex items-start max-w-4xl w-full min-w-0 ${hideAvatar ? "" : "space-x-4"}`}
        >
          ...
          {!hideAvatar &&
            (messages.some((m) => m.isVoiceMessage) ? (
              ...
              <AIIcon
                size={36}
                isAnimating={messages.some((m) => m.isStreaming)}
                className="mt-2"
              />
            ))}
          ...
          <div
            className={`flex-1 space-y-4 pt-1 min-w-0 ${hideAvatar ? "ml-[52px]" : ""}`}
          >
            ...
            {groupedContent.map((item, index) => {
              if (item.type === "tool") {
                ...
                if (index > 0 && groupedContent[index - 1]?.type === "tool")
                  return null;
                ...
                let j = index;
                while (
                  j < groupedContent.length &&
                  groupedContent[j].type === "tool"
                ) {
                ...
                return (
                  <div key={item.key} className="animate-fade-in">
                    <ArtifactNotification
                      ...
                      wordCount={item.artifact.wordCount}
                    />
                  </div>
                );
              }
                          ...
                          return (
                            <div key={idx} className="relative group">
                              ...
                              <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                <Badge
                                  variant="secondary"
                                  className="text-caption bg-black/70 text-white border-0"
                                >
                                  {imageFormat}
                                </Badge>
                              </div>
              ...
              <div className="hidden md:flex justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-200 -mt-1">
                <span className="text-[11px] text-muted-foreground/70">
                  {[
                    ...
                    latencyMetrics?.endToEndLatency &&
                      `E2E ${(latencyMetrics.endToEndLatency / 1000).toFixed(1)}s`,
                    tokenUsage &&
                      `${(tokenUsage.inputTokens / 1000).toFixed(1)}k in · ${tokenUsage.outputTokens} out${
                        (tokenUsage.cacheReadInputTokens ?? 0) > 0 ||
                        (tokenUsage.cacheWriteInputTokens ?? 0) > 0
                          ? ` (${[
                              (tokenUsage.cacheReadInputTokens ?? 0) > 0 &&
                                `${tokenUsage.cacheReadInputTokens!.toLocaleString()} hit`,
                              (tokenUsage.cacheWriteInputTokens ?? 0) > 0 &&
                                `${tokenUsage.cacheWriteInputTokens!.toLocaleString()} write`,
                            ]
                              .filter(Boolean)
                              .join(", ")})`
                          : ""
  ...
  (prevProps, nextProps) => {
    ...
    const messagesEqual =
      ...
      prevProps.messages.every((msg, idx) => {
        ...
        const latencyChanged =
          msg.latencyMetrics?.timeToFirstToken !==
            nextMsg.latencyMetrics?.timeToFirstToken ||
          msg.latencyMetrics?.endToEndLatency !==
            nextMsg.latencyMetrics?.endToEndLatency;
        ...
        const toolExecutionsChanged = prevToolExecs.some((tool, toolIdx) => {
          ...
          if (tool.streamingResponse !== nextTool.streamingResponse)
            return true;
    ...
    const reasoningEqual =
      prevProps.currentReasoning?.text === nextProps.currentReasoning?.text;
    const callbackEqual =
      ...
      prevProps.onOpenExtractedDataArtifact ===
        nextProps.onOpenExtractedDataArtifact &&
    ...
    const researchProgressEqual =
      prevProps.researchProgress?.stepNumber ===
        nextProps.researchProgress?.stepNumber &&
      prevProps.researchProgress?.content ===
        nextProps.researchProgress?.content;

    // Compare codeProgress for real-time code agent status
    const codeProgressEqual =
      (prevProps.codeProgress?.length ?? 0) ===
      (nextProps.codeProgress?.length ?? 0);
