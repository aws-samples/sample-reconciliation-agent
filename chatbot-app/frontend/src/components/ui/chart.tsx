...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // The chart config can originate in (semi-trusted) agent tool results, so every token
  // interpolated into the <style> block is strictly sanitized to a safe CSS charset first — a
  // value like `red;}</style><script>…` is stripped and cannot break out of the CSS context.
  // The sanitized CSS is rendered as a plain text child of <style> (never injected as raw
  // HTML): React sets it as the element's text content, which the browser parses as raw CSS.
  // Because the sanitizers below strip every `<`, `>`, and `&`, there is no markup that could
  // escape the style context.
