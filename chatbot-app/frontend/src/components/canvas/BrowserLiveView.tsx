...
export function BrowserLiveView({
  ...
  const [currentLiveViewUrl, setCurrentLiveViewUrl] = useState<
    string | undefined
  >(undefined);
      ...
      try {
        ...
        try {
          ...
          if (!validateData.isValid) {
            console.log(
              "[BrowserLiveView] Session not valid:",
              validateData.status || validateData.error,
            );
            if (mounted) {
              onValidationFailed?.();
            }
                  ...
                  observers: {
                    ...
                    displayLayout: (
                      ...
                      if (display && display.parentElement) {
                        ...
                        console.log(
                          `[BrowserLiveView] Browser: ${serverWidth}x${serverHeight}, Scale: ${scale.toFixed(3)}`,
                        );
                      }
                    },
                    firstFrame: () => {
                      ...
                      if (connectionRef.current?.requestDisplayLayout) {
                        try {
                          ...
                          console.warn(
                            "[BrowserLiveView] Could not set display layout:",
                            e,
                          );
                        }
                      }
                    ...
                    error: (error: any) => {
                      console.error(
                        "[BrowserLiveView] Connection error:",
                        error,
                      );
                      if (!mounted) return;
                      setError(
    ...
    return () => {
      ...
      if (connectionRef.current && connectionEstablished) {
        try {
          ...
          if (container) {
            // Clear via the DOM API (not innerHTML) so no HTML is ever parsed — avoids the
            // insecure-innerHTML pattern; this only removes the DCV SDK's child nodes.
    ...
    <div className="flex flex-col h-full w-full">
      ...
      <div className="px-4 py-3 border-b border-sidebar-border/50 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-label font-medium text-sidebar-foreground">
            Browser View
          </span>
          {!loading && !error && (
        ...
        {error && (
          <div className="absolute inset-0 flex items-center justify-center bg-background/95 backdrop-blur-sm z-10">
            <div className="text-center max-w-md px-6">
              ...
              <p className="text-heading font-semibold text-destructive mb-2">
                Connection Failed
              </p>
