...
export function Canvas({
  ...
  const handleDownloadMarkdown = () => {
    if (!selectedArtifact || typeof selectedArtifact.content !== "string")
      return;
  ...
  const handlePrintPDF = () => {
    ...
    // The already-rendered preview, serialized (images in it carry resolved presigned URLs).
    // Taking it from the DOM is also what keeps the print window safe: <Markdown> renders through
    // React, which never emits raw HTML from the artifact, so this string is script-free by
    // construction. The previous fallback — `|| marked.parse(content)` — was the opposite: marked
    // passes raw HTML through, so a model-authored artifact containing a <script> tag would run it
    // in a window that inherits this app's origin. There is nothing to print without the preview
    // anyway, so bail out instead.
    const htmlContent = previewContentRef.current?.innerHTML;
    ...
    const printContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>${escapeHtml(selectedArtifact.title)}</title>
        <style>
          ${baseCSS}
          ${citationPrintCSS}
      ...
      <div className="flex-shrink-0 px-4 py-3 border-b border-sidebar-border/50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-heading font-semibold text-sidebar-foreground">
              Canvas
            </span>
            {displayArtifacts.length > 0 && (
              <span className="text-label text-sidebar-foreground/60">
          ...
          {selectedArtifact ? (
            ...
            selectedArtifact.type === "browser" && browserState ? (
              ...
              <>
                ...
                <div className="px-4 py-3 border-b border-sidebar-border/50">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    {(selectedArtifact.type === "document" ||
                      selectedArtifact.type === "research") &&
                ...
                {selectedArtifact.type === "excalidraw" ? (
                  ...
                  <div
                    className={`flex-1 min-h-0 flex flex-col transition-all duration-500 ${justUpdated ? "bg-green-500/10 ring-2 ring-green-500/30 rounded-lg" : ""}`}
                  >
                    <div className="flex-shrink-0 px-3 py-1.5 text-xs text-muted-foreground bg-muted/40 border-b border-border/40 flex items-center gap-1.5">
                      ...
                      <span className="text-muted-foreground/60">
                        Ask the agent to modify, or export (⋮) to save locally.
                      </span>
                    </div>
                    <div className="flex-1 min-h-0">
                      <ExcalidrawRenderer data={selectedArtifact.content} />
                    </div>
                  </div>
                ) : selectedArtifact.type === "word_document" ||
                  selectedArtifact.type === "excel_spreadsheet" ||
                  selectedArtifact.type === "powerpoint_presentation" ||
                  (selectedArtifact.type === "document" &&
                    typeof selectedArtifact.content === "string" &&
                    isOfficeFileUrl(selectedArtifact.content)) ? (
                  // Office document viewer (Word/Excel/PowerPoint) - full height, no ScrollArea
                  <div
                    className={`flex-1 min-h-0 transition-all duration-500 ${justUpdated ? "bg-green-500/10 ring-2 ring-green-500/30 rounded-lg" : ""}`}
                  >
                    <OfficeViewer
                      s3Url={
                        selectedArtifact.type === "word_document" ||
                        ...
                        selectedArtifact.type === "powerpoint_presentation"
                          ? selectedArtifact.content ||
                            selectedArtifact.metadata?.s3_url ||
                            ""
                          : selectedArtifact.content
                      }
                      filename={
                        ...
                        selectedArtifact.type === "powerpoint_presentation"
                          ? selectedArtifact.title
                          : getFilenameFromS3Url(selectedArtifact.content)
                      }
                    />
                  </div>
                ) : (
                  <ScrollArea className="flex-1">
                    <div
                      className={`p-4 transition-all duration-500 ${justUpdated ? "bg-green-500/10 ring-2 ring-green-500/30 rounded-lg" : ""}`}
                    >
            ...
            <Empty className="text-sidebar-foreground/60">
              <EmptyHeader>
                <EmptyMedia
                  variant="icon"
                  className="bg-sidebar-accent text-sidebar-foreground/50"
                >
                  <Layers className="h-6 w-6" />
                </EmptyMedia>
                <EmptyTitle className="text-sidebar-foreground/80">
                  No Content Selected
                </EmptyTitle>
