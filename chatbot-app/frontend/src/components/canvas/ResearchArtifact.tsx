...
export function ResearchArtifact({
  ...
  const handleDownloadMarkdown = useCallback(() => {
    ...
    const blob = new Blob([completedResult.content], {
  ...
  const handlePrintPDF = useCallback(() => {
    ...
    // Serialized from the DOM on purpose: <Markdown> renders through React, which never emits raw
    // HTML from the research text, so this is script-free by construction. Parsing the markdown
    // here instead would hand model-authored HTML to a window that inherits this app's origin.
    ...
    const printContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>${escapeHtml(completedResult.title)}</title>
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-shrink-0 px-6 py-4 border-t border-border/50">
          <div className="flex justify-end items-center gap-2">
            ...
            <Button size="sm" onClick={handleApprove} className="px-5">
  ...
  if (completedResult) {
    return (
      <div className="flex flex-col h-full">
        <div className="flex-shrink-0 p-6 border-b bg-gradient-to-r from-green-500/10 to-emerald-500/10">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <h2 className="text-heading font-bold mb-1">
        ...
        <ScrollArea className="flex-1 p-6">
          <div ref={contentRef}>
            <Markdown sessionId={sessionId}>{completedResult.content}</Markdown>
          </div>
        </ScrollArea>
      </div>
    );
