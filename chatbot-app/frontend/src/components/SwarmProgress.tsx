...
import {
  SwarmProgress as SwarmProgressType,
...
async function handleDocumentDownload(
  ...
  if (!sessionId) {
    console.error(
      "[SwarmProgress] No session ID available for document download",
    );
    return;
  }
 ...
 */
function SharedContextRenderer({
  context,
  sessionId,
}: {
          ...
          {documents.map((doc, idx) => {
            ...
            return (
              <div
                ...
                onClick={() =>
                  handleDocumentDownload(doc.filename, doc.tool_type, sessionId)
                }
              >
        ...
        <div className="flex flex-wrap gap-2">
          {images.map((img, idx) => (
            <div
              ...
              className="group relative flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-all duration-200 cursor-pointer border border-gray-200/50 dark:border-gray-700/50 hover:border-gray-300 dark:hover:border-gray-600"
              onClick={() =>
                handleDocumentDownload(img.filename, "image", sessionId)
              }
            >
...
export function SwarmProgress({
  ...
  const intermediateSteps =
    agentSteps?.filter((step) => step.nodeId !== "responder") || [];

  // Check if there's any content to show in agents section
  const hasAgentContent = intermediateSteps.some(
    (s) =>
      s.reasoningText?.trim() ||
      s.responseText?.trim() ||
                ...
                <div className="mt-2 border-l-2 border-blue-500/30 pl-4 space-y-3 animate-fade-in">
                  {intermediateSteps.map((step, index) => (
                    <AgentStepSection
                      ...
                      step={step}
                      isRunning={
                        !isComplete && index === intermediateSteps.length - 1
                      }
                      sessionId={sessionId}
                    />
                  ))}
...
function AgentStepSection({
  ...
  const hasReasoning =
    step.reasoningText && step.reasoningText.trim().length > 0;
  const hasResponse = step.responseText && step.responseText.trim().length > 0;
  const hasToolCalls = step.toolCalls && step.toolCalls.length > 0;
  const hasHandoff =
    step.handoffMessage && step.handoffMessage.trim().length > 0;
  const hasContext =
    step.handoffContext && Object.keys(step.handoffContext).length > 0;
