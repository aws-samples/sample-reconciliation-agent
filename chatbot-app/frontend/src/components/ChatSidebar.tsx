...
export function ChatSidebar({
  ...
  useEffect(() => {
    setIsMounted(true);
    // Show sign-out only when Entra is configured and not local dev
    const hasEntra = !!(
      process.env.NEXT_PUBLIC_ENTRA_TENANT_ID &&
      process.env.NEXT_PUBLIC_ENTRA_CLIENT_ID &&
      process.env.NEXT_PUBLIC_ENTRA_API_AUDIENCE
    );
    const isLocal =
      window.location.hostname === "localhost" ||
      window.location.hostname === "127.0.0.1";
    setShowSignOut(hasEntra && !isLocal);
  }, []);

  const handleSignOut = async () => {
    try {
      const { signOut } = await import("@/components/AuthWrapper");
      await signOut();
    } catch (error) {
  ...
  // Use custom hooks
  const { chatSessions, isLoadingSessions, deleteSession, deleteAllSessions } =
    useChatSessions({
      sessionId,
      onNewChat,
    });
      ...
      {showConnector ? (
        ...
        <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
          <div className="px-4 pb-2 flex-shrink-0 flex items-center justify-between">
            <span className="text-[13px] font-medium text-sidebar-foreground/60 uppercase tracking-wide">
              Chats
            </span>
      ...
      <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete all your chat sessions. This action
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
