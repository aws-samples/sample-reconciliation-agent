...
import { Eyebrow, Panel } from "@/components/app-ui/ui";
...
function Header({ label, value }: { label: string; value: string | undefined }) {
  ...
  return (
    <>
      <dt className="rc-eyebrow pt-0.5">{label}</dt>
...
export function EmailViewer({ email }: { email: EmailRecord }) {
  return (
    <Panel className="rc-rise p-5">
      ...
      <h2 className="mt-2 text-[15px] font-medium leading-snug text-[var(--rc-ink)]">
        {email.subject}
      </h2>
      <dl className="mt-3 grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 border-b border-[var(--rc-line)] pb-3">
      ...
      <pre className="rc-mono mt-3 whitespace-pre-wrap break-words text-[12px] leading-relaxed text-[var(--rc-ink)]">
        {email.body}
      </pre>
    </Panel>
  );
