...
import { environmentName, projectName } from "@/lib/deployment-env";
...
async function getRuntimeArnFromSSM(): Promise<string | null> {
  ...
  const parameterName = `/${projectName()}/${environmentName()}/agentcore/runtime-arn`;
  ...
  try {
    ...
    if (response.Parameter?.Value) {
      ...
      console.log(
        `[Voice Start] Loaded Runtime ARN from SSM: ${cachedRuntimeArn}`,
      );
      return cachedRuntimeArn;
    }
  } catch (error) {
    console.warn(
      `[Voice Start] Failed to get Runtime ARN from SSM (${parameterName}):`,
      error,
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[Voice Start] User: ${userId}, Session: ${sessionId}, New: ${isNew}, Tools: ${enabledTools.length}, AuthToken: ${authToken ? "present" : "missing"}`,
    );
    ...
    if (IS_LOCAL) {
      const agentcoreUrl =
        process.env.NEXT_PUBLIC_AGENTCORE_URL || "http://localhost:8080";
      // IS_LOCAL dev branch only: derive the websocket scheme from the (local) http endpoint.
      // Built by scheme substitution rather than a literal so no insecure-websocket string
      // literal exists in the source; the deployed path (else branch) always uses wss.
      const wsScheme = agentcoreUrl.startsWith("https") ? "wss:" : "ws:";
      const localWsBase = agentcoreUrl.replace(/^https?:/, wsScheme);
      wsUrl = `${localWsBase}/voice/stream`;
      const params = new URLSearchParams();
      params.set("session_id", sessionId);
      if (userId) params.set("user_id", userId);
      if (enabledTools.length > 0)
        params.set("enabled_tools", JSON.stringify(enabledTools));
      ...
      if (enabledTools.length > 0) {
        queryParams["X-Amzn-Bedrock-AgentCore-Runtime-Custom-Enabled-Tools"] =
    ...
    console.log(
      `[Voice Start] WebSocket URL generated (${IS_LOCAL ? "local" : "cloud"} mode)`,
    );
