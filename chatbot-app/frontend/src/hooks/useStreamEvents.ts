...
import {
  A2A_TOOLS_REQUIRING_POLLING,
  isA2ATool,
  getAgentStatusForTool,
} from "./usePolling";
import { getAccessToken } from "@/lib/access-token";
import { updateLastActivity } from "@/config/session";
import {
  TOOL_TO_DOC_TYPE,
  DOC_TYPE_TO_TOOL_TYPE,
  TOOL_TYPE_TO_DOC_TYPE,
  DocumentType,
} from "@/config/document-tools";
...
interface UseStreamEventsProps {
  ...
  onBrowserSessionDetected?: (
    ...
    browserId: string,
  ) => void; // Callback when browser session is first detected
  ...
  onExcalidrawCreated?: (
    ...
    toolCallId: string,
  ) => void; // Callback when excalidraw diagram is created
...
export const useStreamEvents = ({
  ...
  const handleTextMessageStartEvent = useCallback(
    (event: TextMessageStartEvent) => {
      // Swarm mode: non-responder doesn't create chat messages
      if (swarmModeRef.current.isActive) {
        const currentNode =
          swarmModeRef.current.nodeHistory[
            swarmModeRef.current.nodeHistory.length - 1
          ];
      ...
      textBuffer.startFlushing((bufferedText) => {
        ...
        setMessages((prevMsgs) =>
          prevMsgs.map((msg) =>
            msg.id === streamingId ? { ...msg, text: bufferedText } : msg,
          ),
        );
        ...
        setSessionState((prev) => ({
          ...
          streaming: prev.streaming
            ? { ...prev.streaming, text: bufferedText }
            : null,
        }));
      });
      ...
      setUIState((prevUI) => {
        ...
        return {
          ...
          latencyMetrics: {
            ...
            timeToFirstToken:
              ttft ?? prevUI.latencyMetrics.timeToFirstToken ?? null,
          },
        };
      });
    },
  ...
  const handleTextMessageContentEvent = useCallback(
  ...
  const handleTextMessageEndEvent = useCallback(
        ...
        prevMsgs.map((msg) => {
          if (msg.id === streamingIdRef.current) {
            return {
              ...
              ...(ttft &&
                !msg.latencyMetrics && {
                  latencyMetrics: { timeToFirstToken: ttft },
                }),
            };
          }
          return msg;
  ...
  const handleToolCallStartEvent = useCallback(
    (event: ToolCallStartEvent) => {
      // Track tool in swarm mode for expanded view
      if (
        swarmModeRef.current.isActive &&
        swarmModeRef.current.agentSteps.length > 0
      ) {
        const stepIndex = swarmModeRef.current.agentSteps.length - 1;
        const currentStep = swarmModeRef.current.agentSteps[stepIndex];
        ...
        const updatedStep = {
          ...
          toolCalls: [
            ...
            { toolName: event.toolCallName, status: "running" as const },
          ],
        };
        swarmModeRef.current.agentSteps[stepIndex] = updatedStep;
      ...
      const existingToolIndex = currentToolExecutionsRef.current.findIndex(
  ...
  const handleToolCallArgsEvent = useCallback(
  ...
  const handleToolCallEndEvent = useCallback(
    (event: ToolCallEndEvent) => {
      const accumulated =
        toolInputAccumulatorRef.current[event.toolCallId] || "";
      ...
      const normalizedInput =
        parsedInput === null || parsedInput === undefined ? {} : parsedInput;
        ...
        prevMessages.map((msg) => {
          if (msg.isToolMessage && msg.toolExecutions) {
            const updatedToolExecutions = msg.toolExecutions.map((tool) => {
              ...
              const update: any = { toolInput: normalizedInput };
              if (
                tool.toolName === "skill_executor" &&
                normalizedInput.tool_name
              ) {
                update.toolName = normalizedInput.tool_name;
              }
              return { ...tool, ...update };
  ...
  const handleToolCallResultEvent = useCallback(
    (event: ToolCallResultEvent) => {
      ...
      const toolExecution = currentToolExecutionsRef.current.find(
        (tool) => tool.id === event.toolCallId,
      );
      ...
      if (
        ...
        const displayName =
          SWARM_AGENT_DISPLAY_NAMES[currentNode] || currentNode;
      ...
      if (toolName === "update_artifact" && !isCancelled && onArtifactUpdated) {
        console.log(
          "[useStreamEvents] update_artifact completed, triggering artifact refresh",
        );
        onArtifactUpdated();
      }
      ...
      if (
        ...
        !isCancelled &&
        toolMetadata?.artifactId &&
        onExtractedDataCreated
      ) {
        console.log(
          "[useStreamEvents] browser_extract completed, creating artifact:",
          toolMetadata.artifactId,
        );
        // Parse extracted data from tool result
        const extractedDataMatch = toolOutput?.match(
          /\*\*Extracted Data\*\*:\s*```json\n([\s\S]*?)```/,
        );
        const extractedContent = extractedDataMatch
          ? extractedDataMatch[1].trim()
          : "{}";
        const descriptionMatch = toolOutput?.match(
          /\*\*Description\*\*:\s*(.+)/,
        );
        const title = descriptionMatch
          ? descriptionMatch[1].substring(0, 50)
          : "Extracted Data";
    ...
    async (event: RunFinishedEvent) => {
      ...
      if (messageId) {
        ...
        const resolveDocType = (toolExec: {
          ...
          if (
            toolExec.toolName === "skill_executor" &&
            toolExec.toolInput?.tool_name
          ) {
        ...
        if (usedDocTypes.size > 0 && currentSessionId) {
          try {
            ...
            for (const toolExec of currentToolExecutionsRef.current) {
              ...
              if (!filename || !toolExec.isComplete || toolExec.isCancelled)
                continue;
              ...
              if (docType === "word") wordOutputFilenames.add(filename);
                ...
                pptOutputFilenames.add(filename);
            }
            ...
            const fetchPromises = Array.from(usedDocTypes).map(
              async (docType) => {
                const response = await fetch(
                  `/api/workspace/files?docType=${docType}`,
                  {
                ...
                if (response.ok) {
                  ...
                  if (data.files && Array.isArray(data.files)) {
                    ...
                    if (docType === "excel" && excelOutputFilenames.size > 0) {
                      excelDocumentsForArtifact = files.filter(
                        (f: WorkspaceDocument) =>
                    ...
                    if (
                      docType === "powerpoint" &&
                      pptOutputFilenames.size > 0
                    ) {
                      pptDocumentsForArtifact = files.filter(
                        (f: WorkspaceDocument) =>
            ...
            if (
              excelDocumentsForArtifact.length > 0 &&
              onExcelDocumentsCreated
            ) {
        ...
        if (onExcalidrawCreated) {
          for (const toolExec of currentToolExecutionsRef.current) {
            ...
            const isExcalidrawTool =
              toolExec.toolName === "create_excalidraw_diagram" ||
              (toolExec.toolName === "skill_executor" &&
                toolExec.toolInput?.tool_name === "create_excalidraw_diagram");
            if (isExcalidrawTool) {
              try {
        ...
        if (onDiagramCreated) {
          for (const toolExec of currentToolExecutionsRef.current) {
            ...
            if (
              docType === "diagram" &&
              toolExec.metadata?.s3_key &&
              toolExec.metadata?.filename
            ) {
              onDiagramCreated(
                toolExec.metadata.s3_key,
                toolExec.metadata.filename,
        ...
        // Use workspace documents if fetched — RUN_FINISHED carries no usage/images/documents
        const finalDocuments =
          workspaceDocuments.length > 0 ? workspaceDocuments : [];
        ...
        const ttftValue =
          "ttft" in metrics ? metrics.ttft : metrics.timeToFirstToken;
        const e2eValue =
          "e2e" in metrics ? metrics.e2e : metrics.endToEndLatency;
        ...
        setUIState((prev) => {
          ...
          const e2eLatency = requestStartTime
  ...
  const handleBrowserProgressEvent = useCallback(
    (event: CustomEvent) => {
  ...
  const handleResearchProgressEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
  ...
  const handleOAuthElicitationEvent = useCallback(
    (data: CustomEvent) => {
      const ev = (data as any).value;
      const serviceName =
        ev.message?.match(/^(\w+)\s+authorization/i)?.[1] || "Service";

      console.log(
        `[OAuth Elicitation] Authorization required for ${serviceName}:`,
        ev.authUrl,
      );
      ...
      try {
        ...
        /* quota exceeded — postMessage fallback still works */
      }
  ...
  const isCodeAgentExec = (t: ToolExecution) =>
    ...
    (t.toolName === "skill_executor" &&
      t.toolInput?.tool_name === "code_agent");

  const handleCodeAgentStartedEvent = useCallback(
    (_event: CustomEvent) => {
      setSessionState((prev) => ({
        ...prev,
        codeProgress: [
          {
            stepNumber: 0,
            content: "Code agent started — exploring workspace...",
          },
        ],
      }));
    },
  ...
  const handleCodeAgentHeartbeatEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
      ...
      setSessionState((prev) => {
        ...
        if (
          lastIdx >= 0 &&
          existing[lastIdx].content.startsWith("Working...")
        ) {
          const updated = [...existing];
          updated[lastIdx] = {
            stepNumber: 0,
            content: `Working... (${elapsed}s elapsed)`,
          };
        ...
        return {
          ...
          codeProgress: [
            ...existing,
            { stepNumber: 0, content: `Working... (${elapsed}s elapsed)` },
          ],
        };
      });
    },
  ...
  const handleCodeStepEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
      const step = {
      ...
      if (!activeExec) {
        console.warn(
          "[useStreamEvents] code_step: no active code agent exec found for codeSteps persistence",
        );
      }
      if (activeExec) {
  ...
  const handleCodeTodoUpdateEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
      ...
      if (!activeExec) {
        console.warn(
          "[useStreamEvents] code_todo_update: no active code agent exec found, skipping",
        );
        return;
      }
  ...
  const handleCodeResultMetaEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
      const codeExec = currentToolExecutionsRef.current.find((t) =>
        isCodeAgentExec(t),
      );
      if (!codeExec) return;
  ...
  const handleSwarmNodeStartEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
      ...
      if (!swarmModeRef.current.isActive) {
        ...
        swarmModeRef.current.agentSteps = [
          ...swarmModeRef.current.agentSteps,
          newStep,
        ];
      ...
      console.log(
        "[Swarm] Current agentSteps:",
        JSON.stringify(
          swarmModeRef.current.agentSteps.map((s) => ({
            nodeId: s.nodeId,
            status: s.status,
  ...
  const handleSwarmNodeStopEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
  ...
  const handleSwarmHandoffEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
      console.log(
        "[Swarm] Handoff:",
      ...
      if (swarmModeRef.current.agentSteps.length > 0 && ev.from_node) {
        ...
        if (stepIndex >= 0) {
          ...
          console.log(
            "[Swarm] Handoff context saved to",
            ev.from_node,
            ":",
            ev.context,
          );
          ...
          console.warn(
            "[Swarm] Could not find step for from_node:",
            ev.from_node,
          );
        }
      }
  ...
  const handleSwarmCompleteEvent = useCallback(
    (event: CustomEvent) => {
      const ev = (event as any).value;
      console.log(
        "[Swarm] Complete:",
      ...
      const isNonResponderFinal =
        ev.final_node_id && ev.final_node_id !== "responder";
      ...
      setMessages((prevMsgs) => {
        ...
        if (isNonResponderFinal && ev.final_response) {
          console.log(
            "[Swarm] Creating message from non-responder final:",
            ev.final_node_id,
          );
          return [
            ...prevMsgs,
        ...
        if (lastBotIdx === -1) {
          ...
          if (ev.final_response) {
            console.log(
              "[Swarm] Creating fallback message (no bot message found)",
            );
            return [
              ...prevMsgs,
              {
      ...
      const finalNodeHistory = [
        ...(ev.node_history || swarmModeRef.current.nodeHistory),
      ];
  ...
  const handleStreamEvent = useCallback(
    (event: AGUIStreamEvent) => {
      try {
        ...
        switch (event.type) {
          ...
          case EventType.RUN_FINISHED:
            ...
            handleCompleteEvent(event).catch((err) => {
              console.error(
                "[useStreamEvents] Error in complete event handler:",
                err,
              );
            });
            break;
                ...
                if (ev?.browserSessionId) {
                  setSessionState((prev) => {
                    ...
                    return {
                      ...prev,
                      browserSession: {
                        sessionId: ev.browserSessionId,
                        browserId: ev.browserId || null,
                      },
                    } as ChatSessionState;
                  });
                  if (
                    !sessionState.browserSession &&
                    onBrowserSessionDetected
                  ) {
                    onBrowserSessionDetected(
                      ev.browserSessionId,
        ...
        console.error(
          "[useStreamEvents] Error processing stream event:",
          error,
          "Event type:",
          event?.type,
        );
