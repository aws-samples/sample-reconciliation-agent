...
export const useChat = (props?: UseChatProps): UseChatReturn => {
  ...
  const { startPolling, stopPolling, checkAndStartPollingForA2ATools } =
    usePolling({
      sessionId,
      loadSession: apiLoadSession,
    });
  ...
  useEffect(() => {
    ...
    if (hasOngoingResearch) {
      ...
      // but the A2A agent completed in the background. The stream's complete/error handlers are
      // the only other path back to idle, so without this agentStatus stays 'researching' forever.
      setUIState((prev) => {
        if (prev.agentStatus === "researching") {
  ...
  // ==================== SESSION LOADING ====================
  const loadSessionWithPreferences = useCallback(
    async (newSessionId: string) => {
      ...
      const hasPendingCompact = !!localStorage.getItem(
        `compact_pending_${newSessionId}`,
      );
      ...
      try {
        const { preferences, messages: loadedMessages } =
          await apiLoadSession(newSessionId);
        ...
        console.log(
          `[useChat] ${preferences ? "Restoring session" : "Using default"} preferences:`,
        ...
        try {
          const modelsResponse = await apiGet<{ models: { id: string }[] }>(
            "model/available-models",
          );
          const validModelIds = modelsResponse.models?.map((m) => m.id) || [];
          if (
            validModelIds.length > 0 &&
            !validModelIds.includes(restoredModel)
          ) {
            console.warn(
              `[useChat] Saved model ${restoredModel} not in available models, falling back to default`,
            );
      ...
      try {
        ...
        await fetch("/api/stream/elicitation-complete", {
          ...
          body: JSON.stringify({
            sessionId,
            elicitationId,
            oauthSessionUri: event.data.sessionId,
          }),
        });
      } catch (error) {
        console.error(
          "[useChat] Failed to signal elicitation complete:",
          error,
        );
      }
  ...
  const respondToInterrupt = useCallback(
    async (interruptId: string, response: string) => {
      ...
      const agentStatus: "thinking" | "researching" = isResearchInterrupt
        ? "researching"
        : "thinking";
  ...
  const resumeCompact = useCallback(
    async (sid: string, oldEventIds: string[]) => {
      console.log(
        `[compact] Resuming pending compact for session ${sid} (${oldEventIds.length} events to delete)`,
      );
  ...
  const compactSession = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      localStorage.setItem(
        getCompactPendingKey(currentSessionId),
        JSON.stringify({ oldEventIds }),
      );
  ...
  const truncateFromMessage = useCallback(
    async (message: Message): Promise<void> => {
      ...
      if (!isHistoryMessage && !message.rawTimestamp) {
        console.warn(
          "[truncate] Missing rawTimestamp for non-history message, aborting",
        );
        return;
      }
  ...
  const updateModelConfig = useCallback(
    (modelId: string, temperature?: number) => {
  ...
  const toggleProgressPanel = useCallback(() => {
    setUIState((prev) => ({
      ...prev,
      showProgressPanel: !prev.showProgressPanel,
    }));
  }, []);
  ...
  const addVoiceToolExecution = useCallback((toolExecution: ToolExecution) => {
    console.log(
      `[useChat] addVoiceToolExecution: ${toolExecution.toolName}, id=${toolExecution.id}`,
    );
    ...
    setMessages((prev) => {
      ...
      let updated = prev.map((msg) => {
        if (msg.isVoiceMessage && msg.isStreaming && msg.sender === "bot") {
          console.log(
            `[useChat] Finalizing assistant streaming message before tool: ${msg.id}`,
          );
      ...
      const existingToolMsgIdx = updated.findIndex(
        (msg) =>
          msg.isToolMessage &&
          msg.isVoiceMessage &&
  ...
  const addArtifactMessage = useCallback(
    (artifact: {
      id: string;
      type: string;
      title: string;
  ...
  const finalizeVoiceMessage = useCallback(() => {
    ...
    setMessages((prev) => {
      ...
      const hasStreamingMessages = prev.some(
        (msg) => msg.isVoiceMessage && msg.isStreaming === true,
      );
      ...
      return prev.map((msg) => {
        if (msg.isVoiceMessage && msg.isStreaming === true) {
          ...
          console.log(
            `[useChat] Finalizing ${msg.sender} message: ${msg.id} -> ${finalId}`,
          );
    ...
    (role: "user" | "assistant", deltaText: string, _isFinal: boolean) => {
      ...
      setMessages((prev) => {
        ...
        if (otherStreamingIdx >= 0) {
          ...
          console.log(
            `[useChat] Role changed: finalizing ${otherSender} message: ${otherMsg.id} -> ${finalId}`,
          );
        ...
        const streamingMsgIdx = updatedMessages.findIndex(
          (msg) =>
            msg.isVoiceMessage &&
            msg.isStreaming === true &&
            msg.sender === sender,
        ...
        if (streamingMsgIdx >= 0) {
          ...
          console.log(
            `[useChat] Appending to streaming ${sender} message: id=${existingMsg.id}, newLen=${newText.length}`,
          );
          ...
          console.log(
            `[useChat] Creating NEW voice message for ${sender}: ${newId}, delta="${deltaText.substring(0, 30)}..."`,
          );
