...
export function useVoiceChat({
  ...
  const [currentToolExecution, setCurrentToolExecution] =
    useState<VoiceToolExecution | null>(null);
      ...
      try {
        ...
        switch (data.type) {
          ...
          case "bidi_transcript_stream":
            ...
            console.log(
              `[VoiceChat] Transcript: role=${data.role}, is_final=${data.is_final}, delta="${data.delta?.substring(0, 50) || ""}..."`,
            );
            ...
            if (role === "assistant" && !userHasSpokenRef.current) {
              console.log(
                "[VoiceChat] Ignoring assistant message before user input",
              );
              break;
            }
          ...
          case "bidi_interruption":
            ...
            console.log(
              "[VoiceChat] User interrupted - finalizing assistant message",
            );
          ...
          case "bidi_response_complete":
            ...
            console.log(
              "[VoiceChat] Response complete - finalizing assistant turn",
            );
            onResponseComplete?.();
            onStatusChange("voice_listening");
            break;
          ...
          case "bidi_usage":
            ...
            if (data.inputTokens !== undefined) {
              console.log(
                `[VoiceChat] Usage: input=${data.inputTokens}, output=${data.outputTokens}, total=${data.totalTokens}`,
              );
              onUsageUpdate?.({
  ...
  const endVoiceSession = useCallback(async (sessionIdToEnd: string) => {
    try {
      ...
      const endToken = await getAccessToken();
      if (endToken) {
        headers["Authorization"] = `Bearer ${endToken}`;
      }
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const authToken: string | null = await getAccessToken();
      ...
      if (effectiveSessionId) {
        ...
        console.log(
          "[VoiceChat] No session ID available - BFF will create new session",
        );
      }
      if (authToken) {
      ...
      if (!existingSessionId && activeSessionId) {
        ...
        console.log(
          `[VoiceChat] Synced session ID to sessionStorage: ${activeSessionId}`,
        );
        // Notify parent that a new session was created (refresh session list)
        onSessionCreated?.();
      } else if (existingSessionId !== activeSessionId) {
        console.warn(
          `[VoiceChat] Session ID mismatch - expected: ${existingSessionId}, got: ${activeSessionId}`,
        );
      }
      ...
      ws.onopen = () => {
        ...
        console.log(
          `[VoiceChat] Sent config: session=${activeSessionId}, tools=${enabledTools.length}, authToken=${voiceAuthToken ? "present" : "missing"}`,
        );
      };
  ...
  useEffect(() => {
    ...
    if (
      ...
      sessionId !== connectedSessionIdRef.current
    ) {
      console.log(
        `[VoiceChat] Session changed from ${connectedSessionIdRef.current} to ${sessionId}, disconnecting`,
      );
      disconnect();
    }
