...
export const useMetadataTracking = () => {
  ...
  const recordE2E = useCallback((params: SaveMetadataParams) => {
    ...
    const shouldSave =
      !metadataSavedRef.current &&
      (ttftRef.current || e2e || params.tokenUsage || params.documents);
...
async function saveMetadata(
  ...
  if (!messageId.startsWith("msg-")) {
    try {
      const historyAuthHeaders: Record<string, string> = {
        "Content-Type": "application/json",
      };
      const historyToken = await getAccessToken();
      if (historyToken) {
        historyAuthHeaders["Authorization"] = `Bearer ${historyToken}`;
      }

      const response = await fetch(
        `/api/conversation/history?session_id=${sessionId}`,
        {
          method: "GET",
