/**
 * Browser-side Authorization header for calls to the reconciliation BFF.
 *
 ...
 * matching the existing convention in `src/lib/access-token.ts` (`window.__msal_instance`); we
 * construct one from the same config as a fallback rather than a second parallel client.
 *
 ...
 */
export async function reconIdToken(): Promise<string | null> {
  if (typeof window === "undefined") return null;
  try {
    return PROVIDER === "okta" ? await oktaIdToken() : await entraIdToken();
  } catch (error) {
    console.warn("[ReconAuth] could not read ID token:", error);
    return null;
  }
}

/**
 * Authorization header for the recon BFF, or `{}` when unauthenticated.
 *
 * Returns a spreadable object so callers never branch on the signed-in state.
 */
export async function authHeaders(): Promise<Record<string, string>> {
  const token = await reconIdToken();
