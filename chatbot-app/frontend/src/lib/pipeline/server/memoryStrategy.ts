...
// Memory Manager's read-only strategy panel renders. Copied from the original sample's lessons panel
// `src/lib/memoryStrategy.ts` (same repository, earlier feature) when that module was retired; the
// projection is unchanged because the SDK shape it reads is the same.
//
...
/** Response shape of `GET /api/pipeline/memory/strategy`. */
export interface MemoryStrategyResponse {
  /** False when KNOWLEDGE_MEMORY_ID is unset — the same feature-gate contract the records route uses. */
  configured: boolean;
